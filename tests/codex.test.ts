...
describe('codexHome / codexSessionsDir', () => {
  it('defaults to ~/.codex under the given home', () => {
    expect(codexHome({}, '/home/user')).toBe(path.join('/home/user', '.codex'));
    expect(codexSessionsDir({}, '/home/user')).toBe(path.join('/home/user', '.codex', 'sessions'));
  });

  it('honours CODEX_HOME the way CLAUDE_CONFIG_DIR overrides ~/.claude', () => {
    expect(codexHome({ CODEX_HOME: '/elsewhere/.codex-alt' }, '/home/user')).toBe(
      '/elsewhere/.codex-alt',
    );
  });
});
